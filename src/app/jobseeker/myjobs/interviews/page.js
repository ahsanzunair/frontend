    ...
    <>
      ...
      <div className='w-full py-5 px-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5'>
        {appliedJobs.map((apply) => (
            <div key={apply.id} className='border border- rounded-xl p-5 shadow-sm hover:shadow-md transition bg-white'>
              ...
              <div className='flex items-center justify-start gap-2'>
                <p className="text-black mt-3">Interview Date and Time: <span className='text-[#114A69]'>{apply.interviewDate}</span></p>
                <p className="text-[#114A69] mt-3">{apply.interviewTime}</p>
              </div>
            </div >
        ))}
      </div >
